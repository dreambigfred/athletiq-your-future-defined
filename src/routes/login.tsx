...
function LoginPage() {
  ...
  useEffect(() => {
    if (!loading && user) {
      // User is already logged in — route based on profile
      routeBasedOnProfile(user.id);
    }
  }, [user, loading]);

  async function routeBasedOnProfile(userId: string) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.user) {
        await routeBasedOnProfile(data.user.id);
      }
    } catch (err: any) {
